...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            ...
            with open(load_initial_expertdata, 'rb') as f:
                ...
                for d in loaded_object:                 # loaded_object中有两个trajectories   
                    observation,image_obs,reward,action,next_observation,terminal = d['observation'],\
                        d['image_obs'],d['reward'],d['action'],d['next_observation'],d['terminal']

                    path = utils.Path(observation, image_obs, action, reward, next_observation, terminal)
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            ob_batch, ac_batch = torch.from_numpy(ob_batch).to(ptu.device), torch.from_numpy(ac_batch).to(ptu.device)
